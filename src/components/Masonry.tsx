...
// Patch: Handle videos in preloader
            ...
            {grid.map(item => {
                return (
                    <div
                        key={item.id}
                        data-key={item.id}
                        className="item-wrapper chroma-card"
                        onClick={() => window.open(item.url, '_blank', 'noopener')}
                        onMouseEnter={e => handleMouseEnter(e, item)}
                        onMouseLeave={e => handleMouseLeave(e, item)}
                        style={{
                            // Ensure absolute positioning is respected despite chroma-card class
                            position: 'absolute',
                            left: 0,
                            top: 0
                        }}
                    >
                        {item.type === 'video' ? (
                            <video
                                src={item.img}
                                className="item-img object-cover w-full h-full rounded-[10px]"
                                autoPlay
                                muted
                                loop
                                playsInline
